...
def define_options():
        ...
        parser.add_argument("-V", "--variant", type=int,
                help="Variant position")
        parser.add_argument("-PL", "--plotlabel", type=str,default="sashimi plot",
...
#       args.bam = "/nfs/no_backup/rg/epalumbo/projects/tg/work/8b/8b0ac8705f37fd772a06ab7db89f6b/2A_m4_n10_toGenome.bam"
        ...
        for strand in bam_dict:
                ...
                # Add annotation layer for variant positions
                #variant_positions = 10000  # Example variant positions
                R_script += """
                # Add annotation layer for variant positions
                #gp <- gp +
                #  geom_point(data = data.frame(x = variant_positions, y = 0), aes(x, y),
                #             color = "red", size = 3) +
                #  geom_text(data = data.frame(x = variant_positions, y = 0), aes(x, y, label = "Variant"),
                #            vjust = -1, hjust = 0)
                """
                ...
                for (bam_index in 1:length(density_list)) {
                        ...
                        # Density plot
                        #variant_positions = 157026330
                        variant_positions = %(variant)s
                        ...
                        if(bam_index ==1){
                        gp <- gp + ggtitle("%(plotlabel)s") + theme(plot.title = element_text(hjust = 0.5))
			gp = gp + geom_point(data = data.frame(x = variant_positions, y = 0), aes(x, y),color = "green", size = 5)
                        gp = gp + geom_text(data = data.frame(x = variant_positions, y = 0), aes(x, y, label = "Variant",vjust = -1, hjust = 0))}
		        if(exists('coord_dict')) {
